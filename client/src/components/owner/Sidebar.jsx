...
import me from '../../assets/me.png'  // Direct import of your photo
  ...
  return (
    <div className='relative min-h-screen md:flex flex-col items-center pt-8 max-w-13 md:max-w-60 w-full border-r border-borderColor text-sm'>
      <div className='group relative'>
        <label htmlFor="image">
          <img src={image ? URL.createObjectURL(image) : user?.image || me} alt="" className='h-9 md:h-14 w-9 md:w-14 rounded-full mx-auto' />
      ...
      {image && (
        <button className='absolute top-0 right-0 flex p-2 gap-1 bg-primary/10 text-primary cursor-pointer' onClick={updateImage}>
          Save <img src={assets.save_icon} width={13} alt="" />
        </button>
      )}
      ...
      <div className='w-full'>
        {ownerMenuLinks.map((link, index) => (
          <NavLink key={index} to={link.path} className={`relative flex items-center gap-2 w-full py-3 pl-4 first:mt-6 ${link.path === location.pathname ? 'bg-primary/10 text-primary' : 'text-gray-600'} `}>
            ...
            <div className={`${link.path === location.pathname && 'bg-primary'} w-1 h-full rounded-1 right-0 absolute`}></div>
          </NavLink>
        ))}
      </div>
    </div>
